...
# df2 = pd.read_csv("data/allEstimatesLabeled.csv")


def matrixPlotter(df, phenoClass):
    ...
    connectionOrder = (
        df.groupby(["phenoClass", "connection"])["C"]
        .mean()
    ...
    subdata = df[df.phenoClass == phenoClass]
    ...
    pd.DataFrame(np.nanmedian(mat, axis=1)).to_csv(
        f"formattedData/roiMedianCommon{phenoClass}_Twin.csv"
    )
    ...
    ax.set_ylabel(r"Common Effect", size=8)
    ...
    plt.savefig(
        f"figures/fcs/manhattanCommon{phenoClass}_twin_mag.png",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0,
    )
...
for Type, phenoClass in itertools.product(["twin"], np.unique(df.phenoClass)):
    matrixPlotter(df, phenoClass=phenoClass)
