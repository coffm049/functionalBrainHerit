...
def update_mpheno(iteration, input_json_file = "reExample2.json", estMethod = "AdjHE"):
    output_json_file = f"temp/{estMethod}.FE.{iteration}.json"
    new_mpheno = [f"o{i}" for i in range(iteration * 10, (iteration + 1) * 10 - 1)]

    with open(input_json_file, 'r') as f_in:
        data = json.load(f_in)
        data['mpheno'] = new_mpheno
        data["out"] = "/panfs/jay/groups/31/rando149/coffm049/ABCD/Results/03_heritability/Topography/FCs/probaConns.%s.FE.%s.csv" % (estMethod, iteration)
...
if __name__ == '__main__':
    ...
    update_mpheno(args.iteration, input_json_file = "feExample2.json", estMethod = args.estMethod)
