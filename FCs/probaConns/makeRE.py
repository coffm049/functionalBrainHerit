...
def update_mpheno(iteration, input_json_file = "reExample2.json", estMethod = "AdjHE"):
    ...
    new_mpheno = [f"o{i}" for i in range(iteration * 40, (iteration + 1) * 40 - 1)]

    with open(input_json_file, 'r') as f_in:
        data = json.load(f_in)
        data['mpheno'] = new_mpheno
        data["out"] = "/panfs/jay/groups/31/rando149/coffm049/ABCD/Results/03_heritability/Topography/FCs/probaConns.%s.RE.%s.csv" % (estMethod, iteration)
