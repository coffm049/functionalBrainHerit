...
df = pd.read_csv("data/allEstimatesLabeled.csv")
df.loc[df.h2 > 0.95, "h2"] = 0
df.loc[(abs(df.h2 - 0.5) < 0.01) & (df.Type == "GCTA"), "h2"] = 0
...
def matrixPlotter(df, Type, phenoClass):
    ...
    # filter df to where Type == "GCTA"
    connectionOrder = (
        df.query("Type == 'twin'")
        .groupby(["phenoClass", "connection"])["h2"]
        .mean()
    ...
    mat[np.tril_indices_from(mat, k=-1)] = subdata.h2
    mat = mat + mat.T
    ...
    # ax[0].set_title(f"{phenoClass} {Type} {m} ROIs")
    ...
    ax[1].bar(x=plotdata[0:nlabels].index, height=colored[0:nlabels], color=colors)
    plt.ylim([0, 0.35])
    # remove last ytick from ax[1]
    ...
    ax[1].set_yticklabels(ax[1].get_yticklabels(), size=12)
    # ax.boxplot(plotdata[nlabels:], positions=[nlabels + ], tick_labels = ["other"])
    # ax.boxplot(plotdata[nlabels:], positions=[nlabels], tick_labels = ["other"])
    ax[1].set_ylabel(r"Prop heritable connections", size=12)

    minSigh2 = np.min(subdata[subdata.signif].h2)
    ...
    # set axis limits
    ax[0].set_xlim([0, max(group["index"])])
    ax[0].set_ylim([0, 1])

    ax[0].set_ylabel(r"Heritability ($h^2$)", size=12)
    ax[0].set_yticks([0, 0.25, 0.5, 0.75, 1])
    ax[0].set_yticklabels([0, 0.25, 0.5, 0.75, 1], size=8)
...
for Type, phenoClass in itertools.product(
    ["twin", "GCTA", "AdjHE.RE"], np.unique(df.phenoClass)
):
