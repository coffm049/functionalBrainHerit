...
def reformat2nii(pheno):
    estFile = pd.read_csv("data/allCommonsLabeled.csv").query(
        f"phenoClass == '{pheno}'"
    )
    if pheno == "Gordon":
        ...
        # )
        cifti_template = nib.load("brainTemplate/single_map.dscalar.nii")
    ...
    df = estFile.groupby(["r1"]).agg({"C": lambda x: np.nanquantile(x, 0.9)})
    ...
    for region, value in zip(df.index + 1, df.C):
        # print(region)
    ...
    output_file = f"formattedData/roi90P-{pheno}.dscalar.nii"
    nib.save(new_cifti_img, output_file)

    return None


for ph in ["Gordon", "Proba"]:
    print(ph)
    reformat2nii(pheno=ph)
