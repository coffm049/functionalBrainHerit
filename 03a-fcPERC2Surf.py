...
def reformat2nii(method, pheno):
    ...
    # find 90th percentile grouped by r1
    df = estFile.groupby(["r1"]).agg({"h2": lambda x: np.nanpercentile(x, 90)})
    ...
    output_file = f"formattedData/roi90h2{method}-{pheno}.dscalar.nii"
